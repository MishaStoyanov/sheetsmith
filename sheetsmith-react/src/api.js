...
export async function uploadFile(file, instruction) {
  ...
  const res = await fetch(`${BASE}/api/excel/improve`, { method: 'POST', body: fd });
...
export async function generatePlan(sessionId, instruction) {
  const res = await fetch(`${BASE}/api/excel/plan`, {
    method: 'POST',
...
export async function suggestPlan(sessionId) {
  const res = await fetch(`${BASE}/api/excel/suggest`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function describeSteps(steps) {
  const res = await fetch(`${BASE}/api/excel/describe`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function applyPlan(planToken, steps) {
  const res = await fetch(`${BASE}/api/excel/apply`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getJobStatus(jobId) {
  const res = await fetch(`${BASE}/api/history/${jobId}`);
...
export async function downloadResult(jobId) {
  const res = await fetch(`${BASE}/api/history/${jobId}/download`);
